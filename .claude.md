# Sonic Music - Claude 项目指南

> 版本：v0.0.3-dev | 分支：v0.0.3-dev
> 技术栈：Vue3 `<script setup>` + 纯 JavaScript + 原生 CSS + UniApp

---

## 项目概述

Sonic Music 是一个基于酷狗 API 的音乐播放器，采用 monorepo 结构，包含桌面端（Electron）和移动端（UniApp）两个客户端，共享同一套后端 API 和公共逻辑包。

```
sonic-music/
├── apps/
│   ├── api/              # Express + Kugou API（150+ 接口）端口 6521
│   ├── electron/         # Vue3 + Vite 7 + Pinia + Electron ✅ v0.0.2 已完成
│   └── mobile/           # UniApp Vue3 H5/iOS/Android 🔄 v0.0.3 开发中
├── packages/
│   └── shared/           # 共享 JS 逻辑（utils/stores/composables）
├── docs/architecture/    # 架构文档
├── pnpm-workspace.yaml
└── package.json
```

### apps/mobile 目录结构

```
apps/mobile/
├── index.html            # Vite 入口（必须在根目录，含 <script type="module" src="/src/main.js">）
├── vite.config.js
├── package.json
└── src/
    ├── App.vue           # 根组件：主题初始化 + MiniPlayer 挂载
    ├── main.js           # 入口：注册 pinia
    ├── manifest.json     # UniApp 应用配置
    ├── pages.json        # 路由 + Tab Bar 配置
    ├── uni.scss          # 全局设计 Token（CSS 变量）
    ├── api/
    │   ├── client.js     # uni.request 封装（get/post）
    │   ├── song.js       # 歌曲相关 API
    │   ├── playlist.js   # 歌单 API
    │   ├── search.js     # 搜索 API
    │   ├── ranking.js    # 排行榜 API
    │   └── user.js       # 用户/登录 API
    ├── components/
    │   ├── player/       # MiniPlayer.vue、PlayerControls.vue、ProgressSlider.vue
    │   └── common/       # SongListItem.vue、PlaylistCard.vue、NavBar.vue
    ├── composables/
    │   ├── useAudioPlayer.js  # 音频播放核心（loadAndPlay、pause、seek）
    │   └── useLyrics.js       # 歌词滚动逻辑
    ├── pages/
    │   ├── home/index.vue         ✅ 已完成
    │   ├── search/index.vue       ✅ 已完成
    │   ├── search/result.vue      ✅ 已完成
    │   ├── discover/index.vue     ✅ 已完成
    │   ├── library/index.vue      ✅ 已完成
    │   ├── settings/index.vue     ✅ 已完成
    │   ├── player/index.vue       ✅ 已完成
    │   ├── playlist/detail.vue    ⬜ 待开发
    │   ├── ranking/index.vue      ⬜ 待开发
    │   ├── login/index.vue        ⬜ 待开发
    │   └── artist/detail.vue      ⬜ 待开发
    ├── stores/
    │   └── player.js     # mobile 专用播放器状态
    └── utils/
        ├── storage.js    # uni.getStorageSync 封装
        └── theme.js      # applyTheme(theme) 应用主题
```

---

## 开发命令

```bash
pnpm dev:mobile     # 启动 H5 开发服务器 → http://localhost:5174/
pnpm dev:api        # 启动 API 服务 → http://127.0.0.1:6521
pnpm dev:electron   # 启动 Electron
```

---

## 技术栈约定

### ✅ 必须使用
- Vue 3 `<script setup>` + 纯 JavaScript（不用 TypeScript）
- 原生 CSS scoped + CSS 变量（不用 SCSS/LESS/UnoCSS/Tailwind）
- Pinia（不用 storeToRefs，直接访问 store 属性）
- UniApp API（`uni.request`、`uni.navigateTo`、`uni.getStorageSync` 等）
- rpx 单位（750rpx = 屏幕宽度）

### ❌ 禁止使用
- TypeScript（.ts 文件、类型注解、interface、enum）
- axios（用 `uni.request` 封装的 `api/client.js`）
- localStorage（用 `uni.getStorageSync`）
- HTML 标签（div/span/img/p → 必须用 view/text/image）
- document/window（H5 以外不可用）
- storeToRefs
- uView UI（已移除，用原生 CSS 实现）

---

## packages/shared 内容清单

### 当前已有

```
packages/shared/src/
├── constants/index.js         # QUALITY_MAP、THEME_COLORS、DEFAULT_API_URL
├── utils/
│   ├── cover.js               # getCover(url, size) → 封面图 URL
│   ├── time.js                # formatMilliseconds(ms)、formatSeconds(s)
│   ├── lyrics.js              # parseLyrics(lrc)、getCurrentLineIndex(lyrics, time)
│   ├── quality.js             # getQuality(song, quality) → 音质 hash
│   └── format.js              # formatPlayCount(count) → "1.2万"/"3.4亿"
├── stores/
│   ├── auth.js                # MoeAuthStore（登录态、用户信息）
│   ├── musicQueue.js          # useMusicQueueStore（播放队列）
│   └── settings.js            # useSettingsStore（apiBaseUrl、quality、theme）
└── composables/
    ├── usePlaybackMode.js     # 播放模式切换（顺序/随机/单曲循环）
    └── useQueue.js            # 队列操作（上一首/下一首/插入）
```

### 导入方式

```javascript
import { getCover } from '@sonic-music/shared/utils/cover'
import { formatMilliseconds } from '@sonic-music/shared/utils/time'
import { formatPlayCount } from '@sonic-music/shared/utils/format'
import { useSettingsStore } from '@sonic-music/shared/stores/settings'
import { MoeAuthStore } from '@sonic-music/shared/stores/auth'
import { THEME_COLORS } from '@sonic-music/shared/constants'
```

### 待提取到 shared（下一步）

| 功能 | 当前位置 | 目标 |
|------|----------|------|
| 播放器状态逻辑 | mobile/stores/player.js | shared/stores/player.js |
| 搜索历史 | mobile 本地 | shared/stores/searchHistory.js |

---

## 模块开发顺序（当前状态）

| 模块 | 状态 | 说明 |
|------|------|------|
| 0. packages/shared 基础 | ✅ 完成 | utils/stores/composables |
| 1. mobile 脚手架 + API 层 | ✅ 完成 | client.js + 各 API 模块 |
| 2. 播放器核心 | ✅ 完成 | stores/player + useAudioPlayer + MiniPlayer |
| 3. 主要页面 | ✅ 完成 | home/search/discover/library/settings/player |
| 4. 歌单详情页 | ✅ 完成 | playlist/detail.vue |
| 5. 排行榜页 | ✅ 完成 | ranking/index.vue + ranking/detail.vue |
| 6. 登录页 | ⬜ 待开发 | login/index.vue |
| 7. 歌手详情页 | ⬜ 待开发 | artist/detail.vue |
| 8. 下载功能 | ⬜ 待开发 | stores/download + 下载管理页 |

### 开发节奏规则
**一次一个模块，完成后 commit 再继续。**

---

## UI 对齐流程（Playwright 截图参考）

每个新模块开发前，先截图 Electron 对应页面作为设计参考：

```bash
# 1. 启动 H5 dev server
pnpm dev:mobile   # → http://localhost:5174/

# 2. 用 Playwright 截图（在 Claude 中调用 MCP）
# 截图 Electron 对应页面 → 分析设计语言 → 实现 Mobile 版本
```

截图优先级：
1. PlayerControl → MiniPlayer + 全屏播放器
2. PlaylistDetail → 歌单详情页
3. Home → 首页
4. Search → 搜索页
5. Library → 我的页面

---

## CSS 变量完整列表

```css
/* 主题色（由 applyTheme() 动态设置） */
--primary-color              /* 主色，如 #FF69B4 */
--secondary-color            /* 辅色 */
--background-color           /* 主背景，如 #FFF0F5 */
--background-color-secondary /* 次背景 */
--color-primary              /* 深主色 */
--color-primary-light        /* 主色透明版 */
--border-color               /* 边框色 */
--hover-color                /* 悬停色 */
--color-box-shadow           /* 阴影色 */
--text-primary               /* 主文字色，如 #333 */
--text-secondary             /* 次文字色，如 #999 */
```

### 4 套主题

| 主题 | Primary | Background |
|------|---------|------------|
| pink（默认） | #FF69B4 | #FFF0F5 |
| dark | #BB86FC | #121212 |
| blue | #4A90E2 | #E8F4FA |
| green | #34C759 | #E5F9F0 |

---

## API 调用规范

```javascript
// api/client.js 封装了 uni.request，统一通过它调用
import { get, post } from '@/api/client'

// API 模块定义
export const getRecommend = () => get('/everyday/recommend')
export const getSongUrl = (hash, quality) => get('/song/url', { hash, quality })

// 组件中调用
const res = await getRecommend()
songs.value = res.data?.song_list || res.list || []
```

---

## 路由导航规范

```javascript
uni.navigateTo({ url: '/pages/playlist/detail?id=123' })  // 普通页面（有返回按钮）
uni.switchTab({ url: '/pages/home/index' })               // Tab 页面（必须用 switchTab）
uni.navigateBack()                                         // 返回上一页
uni.reLaunch({ url: '/pages/home/index' })                // 重启到指定页
```

---

## Pinia Store 规范

```javascript
// 定义
export const usePlayerStore = defineStore('player', {
  state: () => ({ currentSong: null, playing: false }),
  actions: {
    setCurrentSong(song) { this.currentSong = song }
  },
})

// 使用（不用 storeToRefs）
const playerStore = usePlayerStore()
playerStore.setCurrentSong(song)
console.log(playerStore.currentSong)
```

持久化用 `uni.getStorageSync`：
```javascript
persist: {
  strategies: [{
    storage: {
      getItem: (key) => uni.getStorageSync(key),
      setItem: (key, val) => uni.setStorageSync(key, val),
      removeItem: (key) => uni.removeStorageSync(key),
    }
  }]
}
```

---

## SFC 代码规范

### 结构顺序：template → script → style

```vue
<template>
  <view class="page">...</view>
</template>

<script setup>
// 1. Vue 核心
import { ref, computed } from 'vue'
// 2. UniApp 生命周期
import { onLoad, onShow } from '@dcloudio/uni-app'
// 3. Stores
import { usePlayerStore } from '@/stores/player'
// 4. 本地组件
import SongListItem from '@/components/common/SongListItem.vue'
// 5. API 函数
import { getRecommend } from '@/api/song'
// 6. 工具函数（shared）
import { getCover } from '@sonic-music/shared/utils/cover'
</script>

<style scoped>
/* rpx 单位，颜色必须用 CSS 变量 */
.page {
  padding: 30rpx;
  background: var(--background-color, #FFF0F5);
  min-height: 100vh;
}
</style>
```

---

## 布局参考

```
Electron（侧边栏）              Mobile（底部导航）
┌──────────────────────┐       ┌──────────────────┐
│  Sidebar  │  Content │  →    │     Content      │
├───────────┴──────────┤       ├──────────────────┤
│     PlayerControl    │       │   MiniPlayer     │
└──────────────────────┘       ├──────────────────┤
                               │    Tab Bar       │
                               └──────────────────┘
```

MiniPlayer 定位（固定在 Tab Bar 上方）：
```css
.mini-player {
  position: fixed;
  bottom: calc(100rpx + env(safe-area-inset-bottom));
  left: 0; right: 0;
  z-index: 999;
}
```

---

## 已知问题与解决方案

### Vite 7 与 @dcloudio/uni-h5-vite 兼容问题

**问题**：electron 使用 Vite 7（ESM-only），uni-h5-vite 用 `require('vite')` 解析到 Vite 7 导致 `ERR_REQUIRE_ESM`。

**解决方案**：
1. `package.json` 中添加 `pnpm.packageExtensions` 声明 dcloudio 包的 vite peer dep
2. `.npmrc` 中 `hoist-pattern[]=!vite` 阻止 vite 提升
3. 在 `node_modules/.pnpm/@dcloudio+uni-h5-vite.../node_modules/` 创建指向 Vite 5 的 junction

**注意**：junction 在 `pnpm install` 后会被清除，需重新创建：
```powershell
New-Item -ItemType Junction -Path 'node_modules/.pnpm/@dcloudio+uni-h5-vite@.../node_modules/vite' -Target 'node_modules/.pnpm/vite@5.4.19_terser@5.46.0/node_modules/vite'
```

### index.html 必须在 apps/mobile/ 根目录

vite 的 root 是 `apps/mobile/`，`index.html` 必须放在这里（不是 `src/`），且必须包含：
```html
<script type="module" src="/src/main.js"></script>
```

### UniApp 条件编译

```javascript
// #ifdef APP-PLUS || MP-WEIXIN
const bgAudio = uni.getBackgroundAudioManager()
// #endif

// #ifdef H5
const innerAudio = uni.createInnerAudioContext()
// #endif
```
